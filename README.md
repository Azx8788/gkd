...
4. **保活体系**（设置 → 保活设置）：
   - 常驻通知 / 无障碍看门狗 / 悬浮窗保活 / 开机自启 四大手段独立开关，自由组合
   - 看门狗：无障碍断开或进程被杀后自动恢复（通知确认 + 闹钟兜底 + Shizuku 自动授权）
   - 内置小米/华为/OPPO/vivo/三星等主流 ROM 的厂商保活手动设置引导
   - 连接特权服务时自动授予 AppOps 后台运行白名单，降低系统后台限制
